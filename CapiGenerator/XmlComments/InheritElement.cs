...
public record InheritElement : SubElementBase
{
    ...
    {
        var itemInheritingFrom = XmlCommentFinder.FindComments(typeProvider, InheritFrom);
        if (itemInheritingFrom == null)
        {
            return;
        }

        var itemToAssignTo = XmlCommentFinder.FindComments(typeProvider, ApplyToLocation);
